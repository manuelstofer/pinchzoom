...
function(a){if(b.enabled){r=!0;d=a.touches.length;var f=(new Date).getTime();1<d&&(e=null);if(300>f-e)switch(h(a),b.handleDoubleTap(a),c){case "zoom":b.handleZoomEnd(a);break;case "drag":b.handleDragEnd(a)}1===d&&(e=f)}});a.addEventListener("touchmove",function(a){if(b.enabled){if(r)l(a),c&&h(a),f=g(a.touches);else{switch(c){case "zoom":b.handleZoom(a,q(f,g(a.touches)));break;case "drag":b.handleDrag(a)}c&&(h(a),b.update())}r=!1}});a.addEventListener("touchend",function(a){b.enabled&&(d=a.touches.length,
l(a))});var m=1;a.addEventListener("mousewheel",function(a){if(b.enabled){var c=Math.max(-1,Math.min(1,a.wheelDelta||-a.detail));m+=c;2>m?(m=1,b.zoomOutAnimation()):b.scale(m,{x:a.offsetX,y:a.offsetY});b.update();h(a)}});var n,p;a.addEventListener("mousedown",function(c){if(b.enabled){n=c.pageX;p=c.pageY;var d=function(b){n=b.pageX;p=b.pageY;a.removeEventListener("mousemove",d);h(b)};a.addEventListener("mousemove",d);var e=function(c){if(n!=c.pageX||p!=c.pageY)b.addOffset({x:n-c.pageX,y:p-c.pageY}),
b.update();a.removeEventListener("mouseup",e);h(c)};a.addEventListener("mouseup",e);h(c)}})};return f};"undefined"!==typeof define&&define.amd?define(["jquery"],function(e){return g(e)}):(window.RTP=window.RTP||{},window.RTP.PinchZoom=g(window.$));(function(e){e.fn.pinchzoom=function(e){e=e||{};return this.each(function(){new RTP.PinchZoom(this,e)})}})(jQuery)}).call(this);
